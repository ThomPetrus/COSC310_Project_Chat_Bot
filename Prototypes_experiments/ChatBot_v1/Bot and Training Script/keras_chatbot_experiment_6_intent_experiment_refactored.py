# -*- coding: utf-8 -*-
"""
Created on Mon Mar  2 21:22:46 2020

    Prototype 6 - Refactor for clarity
        
        There are several semi-redundant seperate methods that perform very similar functions for
        the intent model and the regular model respectively. Obviously this can be refactored to single functions
        that do the the same thing. However due to the differences in the functions, such as number
        and type of arguments sent to each function we have opted to leave these functions as they are
        for legibility and clarity for now.
...
# Global Variables for convenience -- Note that qa_df got overwritten in case this does not work.
model_load_name = '../data/chat_bot_experiment_2500_128_dialogue_dropout_validated_on_train_v3.h5'
model_save_name = '../data/chat_bot_experiment_2500_128_dialogue_dropout_validated_on_train_v3.h5'
intents_model_load_name = '../data/chat_bot_experiment_2500_128_dialogue_dropout_validated_on_train_INTENTS_v3.h5'
intents_model_save_name = '../data/chat_bot_experiment_2500_128_dialogue_dropout_validated_on_train_INTENTS_v3.h5'
...
def train_intents_model(intents_model, questions_train, intents_train):
    ...
    history = intents_model.fit([questions_train, questions_train], intents_train, batch_size=128, epochs=2500, validation_data=([questions_train, questions_train], intents_train))
...
def retrieve_intents_model(max_questions_len, vocab_length, intents_model_questions_train, intents_model_intents_train):
    if input("Create New Intents Model? (Y/N)").lower() in "yes":
        intents_model = build_model(max_questions_len, max_question_len, vocab_length)
