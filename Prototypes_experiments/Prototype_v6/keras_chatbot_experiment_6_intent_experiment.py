...
model_load_name = 'chat_bot_experiment_5000_128_dialogue_dropout_validated_on_train_v2.h5'
model_save_name = 'chat_bot_experiment_5000_128_dialogue_dropout_validated_on_train_v2.h5'
intents_model_load_name = 'chat_bot_experiment_5000_128_dialogue_dropout_validated_on_train_INTENTS.h5'
intents_model_save_name = 'chat_bot_experiment_5000_128_dialogue_dropout_validated_on_train_INTENTS.h5'
...
vocab_save_name = 'dialogue_vocab_v2.txt'
vocab_load_name = 'dialogue_vocab_v2.txt'
tokenizer_save_name = 'dialogue_tokenizer_v2.txt'
tokenizer_load_name = 'dialogue_tokenizer_v2.txt'
...
def load_data_frames():
    ...
    # For the current dialogue data set there is no real point in using a test / train split
...
def train_model(model, intents_train, questions_train, answers_train, is_intents):
    
    # Train the model
    if(is_intents):
        history = model.fit([questions_train, questions_train], answers_train, batch_size=128, epochs=2000, validation_data=([questions_train, questions_train], answers_train))
    else:
    ...
    if(is_intents):
        model.save(intents_model_save_name)
    else:
...
# This can def be refactored further - currently sets up and returns model, vocab, tokenizer and the data frame used.
def setup():
...
# The data loaded depends on what data was processed and saved by the conversion script
    ...
    intents_questions_train, intents_answers_train = vectorize_intents_data(train_data, tokenizer.word_index, max_question_len)
    
    if input("Create New Model? (Y/N)").lower() in "yes":
        model = build_model(max_intent_len, max_question_len, vocab_length)
        model = train_model(model, intents_train, questions_train, answers_train, False)
        intents_model = build_model(max_question_len, max_question_len, vocab_length)
        intents_model = train_model(intents_model, intents_questions_train, intents_questions_train, intents_answers_train, True)
    else:
...
if __name__ == '__main__':
    ...
    btn2 = Button(window, text = "Send", command = questionSelect)
