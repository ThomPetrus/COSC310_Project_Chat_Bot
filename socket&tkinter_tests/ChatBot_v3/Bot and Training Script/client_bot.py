...
def socket_get_bot_answer(model, my_intent, my_question, tokenizer, max_intent_len, max_question_len):
...
def set_bot_img(my_intent, img_panel):

    # Dictionary for mapping the intent names to an image name
    switcher = {
        
        'nan': 'angry',

        'smalltalk.appraisal.good': 'happy',

        'smalltalk.dialog.hug': 'love',

        'smalltalk.agent.marry_user': 'flowers',

        'smalltalk.confirmation.cancel': 'upset',

        'smalltalk.agent.fired': 'surprised',

        'smalltalk.appraisal.bad': 'sad',

        'smalltalk.agent.be_clever': 'cool',

        'smalltalk.agent.boss': 'flowers',

        'smalltalk.agent.my_friend': 'flowers',

        'smalltalk.appraisal.no_problem': 'cool',

        'smalltalk.appraisal.real': 'cool',

        'smalltalk.user.likes_agent': 'happy',
        
        'smalltalk.confirmation.no': 'upset',

        'smalltalk.greetings.hello': 'happy',

        'smalltalk.greetings.bye': 'sad',

        'smalltalk.agent.beautiful': 'love',

        'smalltalk.appraisal.well_done': 'love',

        'smalltalk.user.misses_agent': 'love',

        'smalltalk.agent.clever': 'happy',

        'smalltalk.agent.crazy': 'sad'

        }
        
        
    # Join the my_intent list to be a string and use it to get the right image name from the dictionary above
    img_name = switcher.get(''.join([str(elem) for elem in my_intent]), 'default')

    # Get image from the Images directory
    new_img = ImageTk.PhotoImage(Image.open(images_path + img_name + '.JPG'))

    # Update the image
    img_panel.configure(image = new_img)
    img_panel.image = new_img
...
def socket_print_bot_answer(bot_answer, hst, idx_ans_list):
    if(str(bot_answer).isdigit()):
        #response.configure(text = ' '.join(idx_ans_list[int(bot_answer)-1][1]))
        #hst.insert(INSERT, "Chatbot: " + ' '.join(idx_ans_list[int(bot_answer)-1][1]) + "\n")
        #response.configure(text = ' '.join(idx_ans_list[int(bot_answer)][1][1:]))
        hst.insert(INSERT, "Client: " + ' '.join(idx_ans_list[int(bot_answer)][1][1:]) + "\n")
        conn.send("Client: " + ' '.join(idx_ans_list[int(bot_answer)][1][1:]) + "\n")
...
def init_GUI(model, intents_model, vocab, tokenizer, idx_ans_list, max_intent_len, max_question_len):
    ...
    hst = scrolledtext.ScrolledText(window, width = 100, height = 14, padx=20, pady=20)
    hst.grid(column = 0, row = 0, padx=20, pady=20)

    
    # Response will display all answers and prompts from the chatbot
    response = Label(window, text = "Enter Message:")
    response.grid(column = 0, row = 1, sticky = W, padx=20)
    
    # txt is where the user will query the chatbot
    txt = Entry(window, width = 130)
    txt.grid(column = 0, row = 3, sticky = W, padx=20, pady=20)
    
    
    # Add button widgets to main GUI window and attach functions to them.
    btn = Button(window, text = "Send", command = process_input) 
    btn.grid(column = 0, row = 3, sticky = E, padx=30, pady=20)

    # Get image from the Images directory
    img = ImageTk.PhotoImage(Image.open(images_path + 'default.JPG'))

    # Put image on the top right of the widow
    img_panel = Label(window, image = img)
    img_panel.image = img
    img_panel.grid(column = 1, row = 0, padx=20, pady=20)

  

    return window, txt, hst, response, img_panel

# Similar to set up function this function servers as a collection point for the other method calls.
def generate_answer(txt, hst, response, img_panel, model, intents_model, vocab, tokenizer, idx_ans_list, max_intent_len, max_question_len):
...
# Similar to set up function this function servers as a collection point for the other method calls.
def socket_generate_answer(hst, img_panel, model, intents_model, vocab, tokenizer, idx_ans_list, max_intent_len, max_question_len):
                
    # Retrieve text from server chatbot
    global my_intent
    ...
    # Insert user's original question text in chat window.
    hst.insert(INSERT, "Server: " + my_question_text + "\n")
    ...
    if not my_question:
        ...
        hst.insert(INSERT, "Client: " + unknown_answers[random_index] + "\n")
        
        conn.send("Client: " + unknown_answers[random_index] + "\n")
        ...
        # Predict the Answer based on predicted intent and question / prompt
        bot_answer = socket_get_bot_answer(model, my_intent, my_question, tokenizer, max_intent_len, max_question_len)
           
        socket_print_bot_answer(bot_answer, hst, idx_ans_list)
...
def close_connection():
    conn.send('stop')
    hst.insert(INSERT,"----------CLOSED CONNECTION----------\n")

def process_input():
        generate_answer(txt, hst, response, img_panel, model, intents_model, vocab, tokenizer, idx_ans_list, max_intent_len, max_question_len)

def socket_init_GUI(model, intents_model, vocab, tokenizer, idx_ans_list, max_intent_len, max_question_len):
    ...
    # Add button widgets to main GUI window and attach functions to them.
    btn = Button(window, text = "Step", command = socket_process_input) 
    btn.grid(column = 0, row = 3, sticky = W, padx=30, pady=20)

    # Add button widgets to main GUI window and attach functions to them.
    btn2 = Button(window, text = "Close Connection", command = close_connection) 
    btn2.grid(column = 1, row = 3, pady=20)
    ...
    return window, hst, img_panel
         

def socket_process_input():
        socket_generate_answer(hst, img_panel, model, intents_model, vocab, tokenizer, idx_ans_list, max_intent_len, max_question_len)
...
if __name__ == '__main__':
    ...
    if input("Use Client Chatbot? (Y/N)").lower() in "yes":  
        window, hst, img_panel = socket_init_GUI(model, intents_model, vocab, tokenizer, idx_ans_list, max_intent_len, max_question_len)
        ...
        window, txt, hst, response, img_panel = init_GUI(model, intents_model, vocab, tokenizer, idx_ans_list, max_intent_len, max_question_len)
        window.mainloop()
